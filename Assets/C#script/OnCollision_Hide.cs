...
public class OnCollision_Hide : MonoBehaviour
{
    ...
    public string targetObjectName1; // 목표 오브젝트 이름 ：Inspector에 지정
    ...
    { // 충돌했을 때
      // 만약 충돌한 것의 이름이 목표 오브젝트였다면
        ...
        else if (collision.gameObject.name == targetObjectName2)
        {
            // 지울 오브젝트를 찾아서
            GameObject hideObject = GameObject.Find(hideObjectName);
            hideObject.SetActive(false); // 지운다
        }
        else if (collision.gameObject.name == targetObjectName3)
        {
            // 지울 오브젝트를 찾아서
            GameObject hideObject = GameObject.Find(hideObjectName);
            hideObject.SetActive(false); // 지운다
